...
try {
    ...
    if (!in_array($file['type'], $allowed_types)) {
        http_response_code(400);
        echo json_encode(['success' => false, 'error' => 'Invalid file type. Please upload a JPEG, PNG, GIF, or WebP image.']);
        exit;
    }
    ...
    $filename = 'blog_' . $timestamp . '_' . $random . '.' . $extension;
    ...
    try {
        ...
        $insertStmt->execute([
            ...
            ':file_path' => 'blog-images/' . $filename,
            ':file_size' => $file['size'],
            ':mime_type' => $file['type'],
            ':uploaded_by' => $user_id
        ]);
        ...
        $logStmt->execute([
            ':user_id' => $user_id,
            ':action' => "Uploaded blog image: " . $filename,
        ...
        echo json_encode([
            ...
            'image' => [
                'id' => $image_id,
                'filename' => $filename,
                'url' => "https://mybricklog.s3.us-east-2.amazonaws.com/blog-images/" . $filename,
                'original_filename' => $file['name'],
                'file_size' => $file['size'],
                'mime_type' => $file['type']
            ]
    ...
    error_log("Blog image upload error: " . $e->getMessage());
